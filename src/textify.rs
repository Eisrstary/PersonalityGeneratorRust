...
pub fn to_roleplay(p: &Personality) -> String {
    ...
    o.push_str("【核心性格】\n");
    o.push_str(&core_traits(p));
    o.push_str("\n\n");

    for dom in &Domain::ALL {
        o.push_str(&format!("【{}】\n", dom.name()));
        o.push_str(&domain_desc(p, *dom));
        o.push_str("\n\n");
    }

    o.push_str("【角色扮演提示】\n");
    o.push_str(&role_hints(p));
    o
}
...
pub fn to_compact(p: &Personality) -> String {
    ...
    for i in 0..84 {
        ...
        if dev > 0.3 {
            ...
            o.push_str(&format!("  {} {}: {} ({:.2})\n", PARAMS[i].id, PARAMS[i].name, arrow, p.values()[i]));
        }
    }
    o
}
...
pub fn to_detailed(p: &Personality) -> String {
    ...
    for i in 0..84 {
        ...
        let desc = if p.missing()[i] {
            ...
            let raw = def.range.denormalize(p.values()[i]);
            def.describe_full(raw)
        };
...
fn role_hints(p: &Personality) -> String {
    ...
    let c028 = p.get("C028");
    if c028 > 0.7 { o.push_str("  • 说话风格：独立自主，抵触被指挥\n"); }
    else if c028 < 0.3 { o.push_str("  • 说话风格：顺从配合，接受安排\n"); }

    let b017 = p.get("B017");
    if b017 > 0.7 { o.push_str("  • 情绪表达：容易羞耻，社交失误后长时间纠结\n"); }
    else if b017 < 0.3 { o.push_str("  • 情绪表达：不在意他人眼光，恢复快\n"); }
...
mod tests {
    ...
    fn all_modes_produce_output() {
        let p = Generator::new().from_seed(42, None);
        assert!(to_roleplay(&p).contains("【人格档案】"));
        assert!(to_compact(&p).contains("指纹:"));
        assert!(to_detailed(&p).contains("A001"));
    }
}
